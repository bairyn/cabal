...
main = do
    cabalTest $ do
        ...
        withDirectory "depender" $ do
            ...
            -- First preprocess configuration.
            projectFilePath <- configureDepender env
            packageDbPath   <- liftIO $ guessPackageDbPath

            -- Now test.
            let projectFileArgs = ["--project-file=" ++ projectFilePath]
            let packageDbArgs = ["--package-db=" ++ packageDbPath]
            cabal "v2-configure" $ [] ++ projectFileArgs ++ staticArgs
            cabal "v2-build" $ [] ++ projectFileArgs ++ packageDbArgs
    where
        -- Like 'Test.Cabal.Workdir.guessDistDir' but for ‘dynamic’.
        guessDynamicSdistDir :: IO FilePath
        guessDynamicSdistDir = do
            rootDir <- dropFileName <$> guessDistDir
            canonicalizePath $ rootDir </> "dynamic" </> "dist-newstyle" </> "sdist"

        guessPackageDbPath :: IO FilePath
        guessPackageDbPath = return "/home/bairyn/git/cabal/cabal-testsuite/PackageTests/LinkerOptions/DynDeps/cabal.dist/home/.cabal/store/ghc-9.4.2/package.db"
        ...
        configureDepender env = do
            dynamicSdistDir <- liftIO $ guessDynamicSdistDir
            ...
            let
                ...
                dependerProjectFile = unlines $
                    [
                        ...
                        "    url: file+noindex://" ++ dynamicSdistDir ++ "#shared-cache"
                    ]
